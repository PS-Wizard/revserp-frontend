...
export interface PieProps {
  ...
  selectOnHover?: boolean // dims other sectors while a sector is hovered
  ...
  Children.forEach(children, (child) => {
    ...
    if (type === Pie) {
      ...
      pie = {
        ...
        selectOnHover: props.selectOnHover ?? false,
        labelDataKey,
        labelPosition,
      }
...
function buildPieSeries(ctx: OptionBuildContext): PieSeriesOption[] {
  ...
  const interactive = pie.isClickable || pie.selectOnHover
  ...
  const sectors = data.map((row) => {
    ...
    const isDimmed = interactive && hasSelection && selectedSector !== name
  ...
  return [
    {
      ...
      cursor: pie.isClickable || pie.selectOnHover ? "pointer" : "default",
...
export function EChartsPieChart<TData extends Record<string, unknown>>({
  ...
  live.handlers = {
    ...
    selectOnHover: pie?.selectOnHover ?? false,
    selectedSector,
    selectSector,
  }
  ...
  useEffect(() => {
    ...
    chart.on("mouseover", (params) => {
      const { selectOnHover, selectSector: select } = live.handlers
