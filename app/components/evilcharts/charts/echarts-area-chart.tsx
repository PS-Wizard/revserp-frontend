...
function buildBrushOption(
  ...
  const miniSeries: LineSeriesOption[] = areas.map((area) => {
    ...
    return {
      ...
      xAxisIndex: 1,
      yAxisIndex: 1,
      data: data.map((row) => finiteNumber(row[key])),
...
function buildAreaSeries(ctx: OptionBuildContext): LineSeriesOption[] {
  ...
  const rowTotals = isExpanded
    ? data.map((row) =>
        seriesKeys.reduce((sum, key) => sum + (finiteNumber(row[key]) ?? 0), 0)
      )
    : []
...
function computePlottedTops(
  ...
  const tops: Record<string, Array<number | null>> = {}
  for (const area of areas) {
    const key = area.dataKey
    tops[key] = data.map((row, i) => {
      let value = finiteNumber(row[key])
      if (value === null) return null
...
function resolveAreaAtPixel(
  ...
  tops: Record<string, Array<number | null>>,
  keys: string[],
  x: number,
  y: number
): string | null {
...
type LiveState = {
  ...
  plottedTops: Record<string, Array<number | null>> // per-series plotted line value per index, for pointer hit-testing
